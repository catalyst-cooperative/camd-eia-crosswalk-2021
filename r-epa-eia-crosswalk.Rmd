...
eia_860_year <- 2023 # The  EIA data year (available at https://www.eia.gov/electricity/data/eia860/)
earliest_retirement_year <- 2010 # All units retired before this year are filtered out of the data

if (eia_860_year < 2023){
...
The files used in this analysis are "3_1_Generator_Y{year}.xlsx" and "6_1_EnviroAssoc_Y{year}.xlsx", with "2\_\_\_Plant_Y{year}.xlsx" to get lat/long.
...
get_unmatched <- function(original_data, prev_matches, by = NULL) {
  if (any(missing(prev_matches)) || any(is.na(prev_matches)) || any(is.null(prev_matches))) {
...
Look for matches on the modified EPA and EIA unit identifiers between CAMD and EIA 3_1_Generator (generators) EIA 6_1_EnviroAssoc (boilers) separately.
